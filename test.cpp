...
TEST_F(CollectionTest, MaxSizeGEToEntries) {
...
TEST_F(CollectionTest, CapacityGEToEntries) {
...
TEST_F(CollectionTest, ThrowsOutOfRangeIndexTooBig) {
    add_entries(3);
    EXPECT_THROW(collection->at(5), std::out_of_range);
}

// TODO: Create 2 unit tests of your own to test something on the collection - do 1 positive & 1 negative
// Positive Test: first element can be accessed
TEST_F(CollectionTest, CanAccessFirstElement) {
    add_entries(3);
    EXPECT_NO_THROW({
      int val = collection->at(0);
      EXPECT_GE(val, 0);
      EXPECT_LT(val, 100);
        });
}

// Negative Test: when at() is called and empty, std::out_of_range is thrown
TEST_F(CollectionTest, ThrowsOnEmpty) {
