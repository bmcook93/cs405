...
#include <tuple>
#include <vector>
#include <regex> // regex used for SQL injection check
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // regex used to detect SQL injection patterns
  std::regex injectCheck("\\b(or)\\b\\s*(['\"]?)[^'\"]+\\2\\s*=", std::regex_constants::icase);

  // detect specified SQL injection via regex
  if (std::regex_search(sql, injectCheck)) {
      std::cout << std::endl << "ERROR: SQL Injection Suspected." << std::endl;
      return false;
  }
